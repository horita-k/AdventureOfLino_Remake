...
VS_OUTPUT FireParticleSystem_Vertex_Shader_main(float4 Pos: POSITION){
   ...
   pos.y = particleSpread * s * sin(163 * Pos.z);
   // Particles goes up
   pos.z = particleSystemHeight * t;
...
texture Arrow_Tex0
<
   string ResourceName = "Arrow0.png";
>;
sampler Arrow = sampler_state
{
   Texture = (Arrow_Tex0);
   ADDRESSU = CLAMP;
   ADDRESSV = CLAMP;
...
float4 FireParticleSystem_Pixel_Shader_main(float2 texCoord: TEXCOORD0, float color: TEXCOORD1) : COLOR {
   ...
   return (1 - fade) * tex2D(Arrow, float2(color,0.5f)) * Alpha;
}
